...
有効な二分木
medium
二分木 root が与えられるので、それが二分探索木（BST）かどうか確かめる、validateBST という関数を作成してください。
ただし、有効な BST とは以下の条件を指します。
...
'''
有効な二分木の解説と解答
親ノードのデータと左右の子のデータを比較するため、ヘルパー関数を使って親ノードのデータを渡します。
ヘルパー関数の引数を、root, min, max とし、最初は null を入れておきます。
左の子へ進む場合は親ノードを max の位置に入れて再帰し、右の子へ進む場合は親ノードのデータを min の位置に入れて再帰します。
子が null になるまで進むことができたら true を、途中正しい構造でない場所があったら false を変数に代入します。
最後に二つの変数がどちらも true だったら true を返します。

この再帰の流れを具体的に [5,1,2,3,4] の例で説明します。
この例は正しい構造になっていないので false を返します。
番号はコードのコメントにある番号、インデントは再帰の入れ子を意味します。
...
def validateBSTHelper(root, minValue, maxValue) :
    ...
    # 再帰によって右の子へ進んだ場合、現在のデータよりもminValueに入っている親ノードのデータが大きかったらFalse
    if minValue is not None and minValue >= data: return False # (2)
    # 再帰によって左の子へ進んだ場合、現在のデータよりもmaxValueに入っている親ノードのデータが小さかったらFalse
    if maxValue is not None and maxValue <= data: return False #(3)

    # 現在のノードのdataを渡し再帰的に繰り返します。
    # 葉ノードまでたどり着いたらそれぞれの変数にTrueを代入します。
    left = validateBSTHelper(root.left, minValue, data) if root.left is not None else True # (4)
    right = validateBSTHelper(root.right, data, maxValue) if root.right is not None else True # (5)

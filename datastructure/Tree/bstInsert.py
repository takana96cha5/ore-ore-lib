...
'''
二分探索木への挿入の解説と解答
挿入する key を root と比較して root よりも小さい場合は左に再帰し、そうでない場合は右に再帰します。
葉ノードまで到達したら、key を入れた新しいノードを作って挿入します。それではコードを見てみましょう。
'''
...
def bstInsert(root,key):
    # Noneまでたどり着いた時、またはkeyがBSTに存在した時は、key を入れた新しいノードを作ります。
    if root == None: return BinaryTree(key)

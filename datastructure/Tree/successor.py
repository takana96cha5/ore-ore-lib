...
後続ノードとは木の中に存在する x よりも大きい最小の値を持つノードのことを指します。
...
def successor(root,key):
    ...
    if targetNode.right is not None: return minimumNode(targetNode.right)
...
def minimumNode(root):
    iterator = root
    while iterator is not None and iterator.left is not None: iterator = iterator.left
    return iterator
...
targetNode まで iterator が進んだら、保存していた successor を返します。
'''
